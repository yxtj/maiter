...
static double sigma_degree = exp(_mean_degree)
		* sqrt(exp(2 * _sigma_degree * _sigma_degree) - exp(_sigma_degree * _sigma_degree));
...
static double sigma_weight = exp(_mean_weight)
		* sqrt(exp(2 * _sigma_weight * _sigma_weight) - exp(_sigma_weight * _sigma_weight));
...
static int rand_degree(){
	...
	return static_cast<int>(ceil(lnd(gen)));
}
...
public:
	...
	void gen_hadoop_unweightgraph(TableA* a){
		...
		string patition_file = StringPrintf("%s/part%d", FLAGS_graph_dir.c_str(), current_shard());
		string patition_file_hadoop = StringPrintf("%shadoop/part%d", FLAGS_graph_dir.c_str(),
				current_shard());
		partition.open(patition_file.c_str());
		...
		string delete_cmd = StringPrintf("%s dfs -rmr %s", FLAGS_hadoop_path.c_str(),
				patition_file.c_str());
		string put_cmd = StringPrintf("%s dfs -put %s %s", FLAGS_hadoop_path.c_str(),
				patition_file.c_str(), patition_file.c_str());
		VLOG(1) << "hadoop cmd is " << endl << delete_cmd << endl << put_cmd << endl;
		system(delete_cmd.c_str());
		system(put_cmd.c_str());

		string delete_cmd2 = StringPrintf("%s dfs -rmr %s", FLAGS_hadoop_path.c_str(),
				patition_file_hadoop.c_str());
		string put_cmd2 = StringPrintf("%s dfs -put %s %s", FLAGS_hadoop_path.c_str(),
				patition_file_hadoop.c_str(), patition_file_hadoop.c_str());
	...
	void gen_hadoop_weightgraph(TableA* a){
		...
		string patition_file = StringPrintf("%s/part%d", FLAGS_graph_dir.c_str(), current_shard());
		string hadoop_patition_file = StringPrintf("%shadoop/part%d", FLAGS_graph_dir.c_str(),
				current_shard());
		partition.open(patition_file.c_str());
		...
		string delete_cmd2 = StringPrintf("%s dfs -rmr %s", FLAGS_hadoop_path.c_str(),
				hadoop_patition_file.c_str());
		string put_cmd2 = StringPrintf("%s dfs -put %s %s", FLAGS_hadoop_path.c_str(),
				hadoop_patition_file.c_str(), hadoop_patition_file.c_str());
		VLOG(1) << "hadoop cmd is " << endl << delete_cmd2 << endl << put_cmd2 << endl;
		system(delete_cmd2.c_str());
		system(put_cmd2.c_str());
	}
