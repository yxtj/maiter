...
struct JacobiIterateKernel: public IterateKernel<int, float, vector<Link> > {
	const float zero;
    // static constexpr float Aii = 1.0f;
    // static constexpr float bi = 0.2f;

	JacobiIterateKernel(): zero(0.0f)
    {}
...
static int Jacobi(ConfigData& conf){
	Sharders::Mod vS;
	JacobiIterateKernel vSIK;
