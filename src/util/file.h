...
public:
    ...
    void read_bytes(char* b, int len) {
        if(src_p_) { memcpy(b, src_p_ + pos_, len); } else { f_src_->read(b, len); }
    ...
    bool done() {
        if(src_) { return pos_ == src_->size(); } else { return f_src_->eof(); }
    }
    ...
    void seek(int p) {
        if(src_) { pos_ = p; } else { pos_ = p; f_src_->seek(pos_); }
    }
};
