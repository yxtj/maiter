...
void NetworkThread::ProcessReceivedMsg(int source, int tag, string& data){
	...
	if(h->is_reply){	//Case 1
		...
		VLOG(1)<<"Processing reply, type "<<tag<<", from "<<source<<", to "<<id();
		lock_guard<mutex> sl(rep_lock[tag]);
		...
		if(callbacks_[tag] != NULL){	//Case 2
			...
			VLOG(1) << "Processing RPC, type "<<tag<<", from "<<source<<", to "<<id()<<", content:" << ci->req->ShortDebugString();
			...
			//boost::recursive_mutex::scoped_lock sl(q_lock[tag]);
			lock_guard<mutex> sl(rec_lock[tag]);
...
void NetworkThread::Run(){
	...
	while(running){
		...
		if(net->probe(&hdr)){
			...
			if(hdr.type!=4){
				string s="receive buffer of "+to_string(id())+":\n";
				for(int i=0;i<kMaxMethods;++i){
		...
		while(!pending_sends_.empty()){
			DLOG_IF(INFO,Now()-t>10. && Now()-tt>1.)<<"Wait for mutex send(t). Receive&Process time="<<Now()-tt;
			//boost::recursive_mutex::scoped_lock sl(ps_lock);
			lock_guard<mutex> sl(ps_lock);
			/* bunch send: */
...
bool NetworkThread::CheckQueue(NetworkThread::Queue& q, mutex& m, Message* data){
	if(!q.empty()){
		lock_guard<mutex> sl(m);
