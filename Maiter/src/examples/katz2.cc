...
static int Katz2(ConfigData& conf) {
  ...
  if (!StartWorker(conf)) {
    Master m(conf);
    m.run_all("katz2_RunKernel1", "run", graph);
    m.run_all("katz2_RunKernel2", "map", graph);
    m.run_all("katz2_RunKernel3", "run", graph);
  }
  return 0;
}
REGISTER_RUNNER(Katz2);

class katz2_RunKernel1 : public DSMKernel {
public:
...
REGISTER_KERNEL(katz2_RunKernel1);
REGISTER_METHOD(katz2_RunKernel1, run);

class katz2_RunKernel2 : public DSMKernel {
public:
...
REGISTER_KERNEL(katz2_RunKernel2);
REGISTER_METHOD(katz2_RunKernel2, map);

class katz2_RunKernel3 : public DSMKernel {
public:
	  template <class TableA>
	  void dump(TableA* a){
		  double totalF1 = 0;
...
REGISTER_KERNEL(katz2_RunKernel3);
REGISTER_METHOD(katz2_RunKernel3, run);
