...
static int Adsorption3(ConfigData& conf) {
  ...
  if (!StartWorker(conf)) {
    Master m(conf);
    m.run_all("adsorption3_RunKernel1", "run", graph);
    for(int i=0; i<FLAGS_max_iterations; i++){
        m.run_all("adsorption3_RunKernel2", "map", graph);
        m.run_all("adsorption3_RunKernel3", "map", graph);
    }
    m.run_all("adsorption3_RunKernel4", "run", graph);
  }
  return 0;
}
REGISTER_RUNNER(Adsorption3);

class adsorption3_RunKernel1 : public DSMKernel {
public:
...
REGISTER_KERNEL(adsorption3_RunKernel1);
REGISTER_METHOD(adsorption3_RunKernel1, run);

class adsorption3_RunKernel2 : public DSMKernel {
public:
...
REGISTER_KERNEL(adsorption3_RunKernel2);
REGISTER_METHOD(adsorption3_RunKernel2, map);

class adsorption3_RunKernel3 : public DSMKernel {
public:
	  template <class TableA>
	  void run_loop(TableA* a) {
		  int update = 0;
		  ...
		  const int num_shards = graph->num_shards();
			...
			update++;
                ...
                cout << "iter " << iter << " elapsed " << iter_timer.elapsed() << " shard " << current_shard() <<
                                        " total F2 : " << StringPrintf("%10.5f", totalF2) <<
                                        " update " << update << endl;
	  }
...
REGISTER_KERNEL(adsorption3_RunKernel3);
REGISTER_METHOD(adsorption3_RunKernel3, map);

class adsorption3_RunKernel4 : public DSMKernel {
public:
	  template <class TableA>
	  void dump(TableA* a){
		  fstream File;
...
REGISTER_KERNEL(adsorption3_RunKernel4);
REGISTER_METHOD(adsorption3_RunKernel4, run);
