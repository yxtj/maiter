...
INIT_FDR=$PRE/ref/${FOLDER}-mst
DELTA_FDR=$PRE/delta/$FOLDER/mst/

LOG_FDR=../log/mst/delta/
...
./test-kernel-ncr.sh MinSpanTree $PARTS $GRAPH_FDR $INIT_FDR $DELTA_FDR $RESULT_FDR $LOG_FDR $SNAPSHOT\
  "$DELTA_RATIOS" "$GOOD_RATIOS" "$EW_RATIOS" $K_START $K_END "$PORTIONS" $DIFF "$ALPHAS" $DEGREE $N
